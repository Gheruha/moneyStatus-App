...
export async function POST({ request }) {
	...
	let results = await mysqlConnection
		.query(
			'select sum(case when income is not null then income else 0 end) as total_income,   sum(case when expense is not null then  expense else 0 end) as total_expense,  sum(case when income is not null then income when expense is not null then - expense else 0 end) as total from money where month(day_id) = ' +
				month +
				'and year(day_id) = ' +
				year
		)
